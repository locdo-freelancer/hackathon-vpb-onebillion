...
def print_banner():
    ...
    if platform.system() == "Darwin":
        print("  ✓ macOS SSH connection monitoring (2-second intervals)")
    print("=" * 60)
    print()


def start_macos_log_streaming():
    """Start real-time SSH connection monitoring for macOS (background thread)"""
    if platform.system() != "Darwin":
        return

    def monitor_ssh_connections():
        """Background function to monitor SSH connections on macOS"""
        try:
            print("🍎 Starting macOS SSH connection monitor...")
            print(
                "   (Note: macOS Sequoia doesn't log auth failures, monitoring connections instead)")
            ...
            while True:
                try:
                    # Check for SSH connections every 2 seconds
                    result = subprocess.run(
                        ['lsof', '-i', ':22', '-n', '-P'],
                        capture_output=True,
                        text=True,
                    ...
                    if result.returncode == 0:
                        lines = result.stdout.split('\n')
                        # Count ESTABLISHED or recent connections
                        active_connections = [
                            l for l in lines
                            if any(state in l for state in ['ESTABLISHED', 'CLOSE_WAIT', 'TIME_WAIT', 'SYN_SENT'])
                        ]

                        current_count = len(active_connections)

                        # Detect new connections (excluding listener)
                        if current_count > last_connection_count and current_count > 0:
                            print(
                                f"🔴 [MONITOR] New SSH connection detected ({current_count} active)")
                            ...
                            # Check if threshold reached (5 connections in last 2 minutes)
                            recent_connections = [
                                c for c in connection_history
                                if datetime.now() - c['time'] < timedelta(minutes=2)
                            ]
                            ...
                            if len(recent_connections) >= 5:
                                ...
                                for conn in recent_connections:
                                    ip_counts[conn['ip']] = ip_counts.get(
                                        conn['ip'], 0) + 1

                                for ip, count in ip_counts.items():
                                    if count >= 5:
                                        print(
                                            f"🚨 [MONITOR] Threshold reached for {ip}! Reporting...")
                                        threat = {
                                            'indicator': ip,
                                            'type': 'ip',
                                            'severity': 'high',
                                            'description': f"Suspicious SSH activity from {ip} ({count} connections in 2 minutes)",
                                            'confidence': 85,
                                            'tags': ['ssh', 'suspicious-activity', 'real-time', 'macos'],
                                            'raw_logs': [c['line'] for c in recent_connections if c['ip'] == ip][:5],
                                            'metadata': {
                                                'connection_count': count,
                                                'detection_method': 'connection_monitor',
                                                'note': 'macOS Sequoia does not log SSH auth failures'
                                            }
                                        }
                                        report_threat(threat)
                                        # Clear history for this IP
                                        connection_history = [
                                            c for c in connection_history if c['ip'] != ip]
            ...
            print(f"⚠️  SSH monitor error: {e}")

    # Start monitoring in background thread
    monitor_thread = threading.Thread(
        target=monitor_ssh_connections, daemon=True)
    monitor_thread.start()
    print("✅ macOS SSH connection monitoring started\n")
