...
export class ThreatsController {
  ...
  async create(@Body() createThreatIndicatorDto: CreateThreatIndicatorDto) {
    // Create threat indicator in database first
    const threat = await this.threatsService.create(createThreatIndicatorDto);

    // Trigger AWS workflow for threat processing
    try {
      const workflowResult = await this.workflowService.processThreatDetection({
        threatId: threat.id,
        siteId: createThreatIndicatorDto.site_id || "default",
        indicator: createThreatIndicatorDto.indicator,
        type: createThreatIndicatorDto.type,
        confidence: createThreatIndicatorDto.confidence || 0.5,
        severity: createThreatIndicatorDto.severity,
      });
      ...
      return {
        ...
        workflow: {
          ...
          enrichmentRequestId: workflowResult.enrichmentRequestId,
          status: "initiated",
        },
      };
    } catch (workflowError) {
      // Log error but don't fail the threat creation
      console.error("Failed to initiate threat workflow:", workflowError);
      return threat;
    }
  }
