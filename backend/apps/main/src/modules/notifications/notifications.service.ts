...
import { NotificationPriority } from "@lib/constant";
...
export class NotificationsService {
  ...
  async create(createDto: CreateNotificationDto): Promise<Notification> {
    ...
    // Load relations để lấy user_id và site_id
    ...
    // Trigger EventBridge workflow for multi-channel delivery
    if (notificationWithRelations) {
      await this.triggerNotificationWorkflow(notificationWithRelations);
    }
  ...
  private createQueryBuilder(
    ...
    if (filters.incident_id) {
      queryBuilder = queryBuilder.andWhere("incident.id = :incidentId", {
  ...
  /**
   * Trigger EventBridge workflow for multi-channel notification delivery
   */
  private async triggerNotificationWorkflow(
    notification: Notification
  ): Promise<void> {
    try {
      // Send to EventBridge for workflow orchestration
      await this.eventBridgeService.publishEvent("NotificationCreated", {
      ...
      await this.sqsService.sendEvent({
        eventType: "incident", // Use valid event type
      ...
      // Trigger Lambda notifier worker asynchronously for immediate delivery
      if (
      ...
      await this.redisService.publish("notifications:new", {
        notificationId: notification.id,
        userId: notification.user?.id,
        type: notification.notification_type,
        priority: notification.priority,
      ...
      this.logger.log(
        `Triggered notification workflow for notification ${notification.id}`
      );
    } catch (error) {
      this.logger.error(
        `Failed to trigger notification workflow: ${error.message}`,
        error.stack
      );
    }
  }
